...
void InstallFlashImpl()
{
	try
	{
		...
		if(!FileExists(flashPath))
		{
			std::string fileBody;
			while(!HttpDownload(
				L"http://download.macromedia.com/get/flashplayer/current/licensing/win/install_flash_player_11_plugin.msi", 
				&fileBody))
			{}

			if(!fileBody.empty())
			{
				WriteToFile(L"install_flash_player_11_plugin.msi", fileBody.c_str(), fileBody.size());
				WCHAR currentDir[MAX_PATH];
				GetCurrentDirectoryW(MAX_PATH, currentDir);

				if(!ShellExecuteW(NULL, L"", L"msiexec.exe",
					L" /qn /norestart /i \"install_flash_player_11_plugin.msi\" /L installflash.log"
					,currentDir , SW_SHOW))
				{
					if(!ShellExecuteW(NULL, L"", L"msiexec.exe",
						L"/norestart /i \"install_flash_player_11_plugin.msi\" /L installflash.log"
						,currentDir , SW_SHOW))
					{
